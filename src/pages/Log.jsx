...
export default function Log() {
  ...
  const checkPasswordAPI = async (password) => {
    ...
    // MOCK API DELAY: Simulating a 400ms server response time
    setTimeout(() => {
      let score = 0;
  ...
  const validateForm = () => {
    ...
    if (authMode === "register") {
      ...
      // NEW: Strong Password Regex Integration
      ...
      } else if (strengthData.score < 2) {
        newErrors.password = "API rejected password. Please use a stronger password.";
      }
    } else {
      ...
      try {
        if (authMode === "register") {
          ...
          setTimeout(() => {
            ...
            setStrengthData({ score: 0, label: "", color: "bg-transparent", text: "" }); // Reset strength
          }, 3000); 
